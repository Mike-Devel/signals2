...
namespace boost {
  namespace signals2 {
    namespace detail
    {
        ...
        // trackable_pointee is used to identify the tracked std::shared_ptr
        // originating from the signals2::trackable class.  These tracked
        // std::shared_ptr are special in that we shouldn't bother to
    ...
    private:
      ...
      {
          return std::weak_ptr<detail::trackable_pointee>(_tracked_ptr);
      }
